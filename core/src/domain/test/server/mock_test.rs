...
mod tests {
    ...
    async fn test_get_server_success() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let created_server = service.server_repository.insert(input).await?;
    ...
    async fn test_list_servers_success() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 1..=3 {
            ...
            service.server_repository.insert(input).await?;
        }
    ...
    async fn test_delete_server_success() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let deleted_server = service
            .server_repository
            .find_by_id(&created_server.id)
            .await;
