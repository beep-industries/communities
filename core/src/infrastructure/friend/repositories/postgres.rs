...
use crate::domain::{
    common::{CoreError, GetPaginated}, friend::{entities::{AcceptFriendRequestInput, CreateFriendRequestInput, DeclineFriendRequestInput, DeleteFriendInput, DeleteFriendRequestInput, Friend, FriendRequest, UserId}, ports::FriendshipRepository}
};
...
impl FriendshipRepository for PostgresFriendshipRepository {
    async fn list_friends(&self, pagination: &GetPaginated, user_id: &UserId) -> Result<(Vec<Friend>, u64), CoreError> {
        let offset = (pagination.page - 1) * pagination.limit;

        let total_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM friends WHERE user_id_1 = $1 OR user_id_2 = $1")
            .bind(user_id.0)
            .fetch_one(&self.pool)
            .await
    ...
    async fn remove_friend(
        ...
        if result.rows_affected() == 0 {
            return Err(CoreError::FriendshipDataError);
        }
        Ok(())
    }
    ...
    async fn list_requests(
        ...
        let total_count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM friend_requests WHERE user_id_requested = $1")
            .bind(user_id.0)
            .fetch_one(&self.pool)
            .await
            .map_err(|_| CoreError::FriendshipDataError)?;
    ...
    async fn accept_request(
        ...
        tx.commit().await.map_err(|_| CoreError::FriendshipDataError)?;
